...
Select
	Case $iMsgBoxAnswer = 6 ;Yes=3
		_MainScript()
		FileWrite($LLGAu3, "Func _PixelSearch_Loop($left, $top, $right, $bottom, $color, $shade_variation = 0, $step = 1, $hWND = 0)" & @CRLF & "MsgBox(0, """", ''Press ""["" to start and ""`"" to exit. '')" & @CRLF & "While 1" & @CRLF & "Local $PS = PixelSearch($left, $top, $right, $bottom, $color, $shade_variation, $step, $hWND)" & @CRLF & "If Not @error Then ExitLoop" & @CRLF & "WEnd" & @CRLF & "Return $PS" & @CRLF & "EndFunc" & @CRLF & "HotKeySet(""["", ""_Start"")" & @CRLF & "HotKeySet(""`"", ""_Exit"")" & @CRLF & "While 1" & @CRLF & "Sleep(100)" & @CRLF & "WEnd" & @CRLF & "Func _Exit()" & @CRLF & "Exit" & @CRLF & "EndFunc" & @CRLF & "Func _Start()" & @CRLF & "While 1" & @CRLF & "MouseClick("""", " & $Trainer & ", 75)" & @CRLF & "Send(""{ENTER 5}"")" & @CRLF & _
